...
class ShoreFollowerObserve {
    protected:
        ...
        unsigned long type_counter_[4]; // backward, straight
    ...
    protected: // ROS Callbacks
        ...
        void image_callback(const sensor_msgs::ImageConstPtr & img_msg) {
            ...
            bool save_it = true;
            int label;
            if (last_command_.angular.z<0 && last_command_.linear.x>0){
              label = 0; // avant droite
            }
            else if (last_command_.angular.z>0 && last_command_.linear.x>0){
              label = 1; // avant gauche
            }
            ...
            save_it = (last_command_.linear.x!=0);
            ...
            if (save_it)  {
                ...
                fclose(fp);
                type_counter_[label]++;
                image_counter_++;save_it = (last_command_.linear.x!=0);
            }
            printf("Image counter at %ld (%ld %ld %ld %ld)\n",image_counter_,
                    type_counter_[0],type_counter_[1], type_counter_[2], type_counter_[3]);
        }
    ...
    public:
        ShoreFollowerObserve() : nh_("~"), it_(nh_) {
            ...
            image_counter_ = 0;
            type_counter_[0] = type_counter_[1] = type_counter_[2]= type_counter_[3]=0;
